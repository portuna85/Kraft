...
export const REVALIDATE_ROUNDS_LIST = 300; // 회차 목록: 5분
export const REVALIDATE_ROUND_DETAIL = 3600; // 개별 회차: 1시간
...
// 이름이 일치해야 한다(언어가 달라 완전한 공유는 못하지만, 태그 개수가 고정(4종)이라
// 경로 화이트리스트보다 드리프트 리스크가 훨씬 낮다).
export const TAG_ROUNDS_LATEST = "rounds:latest";
export const TAG_ROUNDS_LIST = "rounds:list";
export const TAG_STATS = "stats:all";
export const tagRoundDetail = (round: number) => `rounds:detail:${round}`;
